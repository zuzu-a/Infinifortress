    ...
    public class CaveGenerator
    {
        ...
        public static BlockType[, ,] GenerateCaveSystem(int size, bool includeLava, uint oreFactor, bool includeWater)
        {
            ...
            for (int i = 0; i < cavesToCarveSand; i++)
                CaveGenerator.PaintWithRandomWalk(ref caveData, ref caveNoiseSand, size/4, randGen.Next(1, 2), BlockType.Sand, false);
        ...
        public static void AddRocks(ref BlockType[, ,] data, int size)
        {
            ...
            for (int i = 0; i < numRocks; i++)
            {
                ...
                int walklen = randGen.Next(1, 10);
                int wx = randGen.Next(1, 3)-1;
                int wz = randGen.Next(1, 3)-1;
                
                for (int w = 0; w < walklen; w++)
                {
                    PaintAtPoint(ref data, x+wx*w, y, z+wz*w, size, rockSize/2, BlockType.Rock);
                }
        ...
        {
            int numFlows = randGen.Next(size / 16, size / 8);
            while (numFlows > 0)
            {
                int x = randGen.Next(0, size);
                int y = randGen.Next(0, size);
